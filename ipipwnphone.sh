...
# Check if iPhone filesystem is detected
if [ $(lsblk -o name,fstype | grep -c -E '(/dev/sd[a-z]|/dev/mmcblk[0-9])\s+(apfs|hfs\+)') -eq 0 ]; then
    echo "No iPhone filesystem detected. Exiting."
    exit
fi
